...
class Force {
    ...
    static Force getForce_h2w(Human H, Wall W) {
        ...
        double dHW;
        if (W.IsLevel && H.location.x_loc >= W.begin.x_loc && H.location.x_loc <= W.end.x_loc)
            dHW = Math.abs(W.begin.y_loc - H.location.y_loc);
        else if (!W.IsLevel && H.location.y_loc >= W.begin.y_loc && H.location.y_loc <= W.end.y_loc)
            dHW = Math.abs(W.begin.x_loc - H.location.x_loc);
        else dHW = 1;
